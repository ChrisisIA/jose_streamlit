...
def save_response():
    try:
        ...
        path_image = data.get("ruta_image")
        number_chat = data.get("numbchat")

        if not all([codigo_usuario, nombre_usuario, pregunta, respuesta, path_image, number_chat]):
        ...
        query = """
        INSERT INTO prdousuahist (TUSUACODI, TUSUANOMB, TUSUACONS, TNETTRESP, TRUTAIMAG, TNUMBCHAT)
        VALUES (%s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (codigo_usuario, nombre_usuario, pregunta, respuesta, path_image, number_chat))
        conn.commit()
...
@app.route('/save_chat_hist', methods=['POST'])
def save_chat_hist():
    try:
        data = request.json
        usercode = data.get("usercode")
        number_of_chat = data.get("numbchat")
        description_chat = data.get("descchat")

        if not all([usercode, number_of_chat, description_chat]):
            return jsonify({"error": "Faltan datos"}), 400
        ...
        # Insertar los datos en la tabla de forma segura
        query = """
        INSERT INTO prdochathist (TUSUACODI, TNUMBCHAT, TDESCCHAT)
        VALUES (%s, %s, %s)
        """
        cursor.execute(query, (usercode, number_of_chat, description_chat))
        conn.commit()
...
def save_prompt():
    try:
        ...
        if not all([codigo_usuario, nombre_usuario, prompt]):
            return jsonify({"error": "Faltan datos"}), 400
        ...
        # Insertar los datos en la tabla de forma segura
        query = """
        INSERT INTO prdoreusprom (TUSUACODI, TUSUANOMB, TCONSPROM)
        VALUES (%s, %s, %s)
        """
        cursor.execute(query, (codigo_usuario, nombre_usuario, prompt))
        conn.commit()
